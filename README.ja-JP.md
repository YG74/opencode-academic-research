...
- **10 スラッシュコマンド**（`commands/ars-*.md`、Phase 2.1、PR #69）が `MODE_REGISTRY.md` エントリーを `/ars-<mode>` トリガーにマッピング。OpenCode ではセッションモデルが各コマンドに使用されるモデルを決定する。`full` および `revision-coach` などの深いモードには `kimi-for-coding/k2p7`（Kimi K2.7）を推奨し、その他の軽量モードには `vllm/qwen3.6`（ローカル Qwen3.6-27B）を使用できる。
...
- **`model: inherit`** がそれら 3 つのソースエージェント frontmatter に追加。`inherit` を選択したのは、ARS フルパイプラインを実行している Kimi K2.7 セッションがエージェントも Kimi K2.7 のまま実行できるようにするため。
