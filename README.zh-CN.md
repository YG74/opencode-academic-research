...
- **10 个 slash command** 在 `commands/ars-*.md`（Phase 2.1，PR #69）将 `MODE_REGISTRY.md` 的条目映射到 `/ars-<mode>` 触发。OpenCode 中由会话模型决定指令所用的模型；`full` 与 `revision-coach` 等深度模式建议使用 `kimi-for-coding/k2p7`（Kimi K2.7），其余轻量模式可使用 `vllm/qwen3.6`（本地 Qwen3.6-27B）。
...
- **`model: inherit`** 加在这三个 source agent frontmatter 上。选 inherit 是为了让 Kimi K2.7 session 跑 ARS full pipeline 时 agent 仍使用 Kimi K2.7（不被降）。
