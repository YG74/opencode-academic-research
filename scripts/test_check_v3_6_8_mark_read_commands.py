...
class TestMarkReadCommandsLint(unittest.TestCase):
    ...
    def test_missing_sonnet_model_routing_fails(self) -> None:
        ...
        MUST declare a non-Haiku model. The OpenCode port accepts either
        the upstream `model: sonnet` or the mapped `model: vllm/qwen3.6`;
        `model: haiku` must still fail."""
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _copy_repo_skeleton(root)
            cmd = root / "commands" / "ars-mark-read.md"
            text = cmd.read_text(encoding="utf-8")
            mutated = text.replace("model: vllm/qwen3.6", "model: haiku")
            cmd.write_text(mutated, encoding="utf-8")
