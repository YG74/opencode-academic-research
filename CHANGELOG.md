...
## [3.13.0-opencode.1] - 2026-06-18 — OpenCode port sync to upstream v3.13.0

### Added

- **OpenCode port sync to upstream v3.13.0.** Merged upstream `Imbad0202/academic-research-skills` v3.13.0 content into the OpenCode port fork (`YG74/opencode-academic-research`). Preserved port-only packaging (`opencode.json`, `package.json`, `pyproject.toml`, `tsconfig.json`, `install.sh`, `plugins/ars-session-loaded.ts`, `AGENTS.md`, `MIGRATION.md`, `docs/OPENCODE_NOTES.md`, `.github/workflows/*`). Converted upstream command frontmatter (`model: sonnet` / `model: inherit`) to port style (`agent: build`, `compatibility: opencode`). Added `license: CC-BY-NC-4.0`, `compatibility: opencode claude-code`, and `allowed-tools` to skill `SKILL.md` frontmatters. Rebranded README, QUICKSTART, CONTRIBUTING, SETUP, PERFORMANCE, and localized READMEs for OpenCode install/setup language. Upstream functional content (skills, agents, scripts, shared, examples, tests, docs/design) is otherwise unchanged.
