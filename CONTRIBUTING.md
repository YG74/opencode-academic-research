...
ARS uses the standard **fork-and-PR** workflow. Fork the repo on GitHub, clone your fork, create a branch, make your changes, push to your fork, then open a PR against `YG74/opencode-academic-research`.

**Scope guidance**: This repo is the OpenCode *port* of the upstream workflow. Changes to core workflow content (agent prompts, schemas, shared references, Python verification logic) are usually better submitted to [`timpara/academic-research-skills`](https://github.com/timpara/academic-research-skills) or the original [`Imbad0202/academic-research-skills`](https://github.com/Imbad0202/academic-research-skills) first, then synced here. OpenCode-specific packaging changes (`opencode.json`, `install.sh`, `plugins/`, `.github/workflows/`, port docs) belong in this repo.
...
This repository is the **OpenCode port** of ARS. The upstream reference distribution ([`Imbad0202/academic-research-skills`](https://github.com/Imbad0202/academic-research-skills)) targets Claude Code. Ports to *other* agent platforms (Claude Code, Cursor, Continue, Aider, etc.) are accepted as community-maintained contributions. Two structural shapes are acceptable — both keep core ARS content as the source of truth:

- **In-tree wrapper.** Add a top-level `<platform>/` directory in this repo (e.g. `cursor/`) containing the manifest, plugin entry, and dispatch shims. Core ARS files (`skills/*/SKILL.md`, `agents/*.md`, `shared/`, `scripts/`) remain unmodified.
...
- **Model-portability note.** ARS prompts were originally calibrated against Claude (Opus for architecture/review, Sonnet for execution; never Haiku). The PR must document which providers/models were tested on OpenCode and where downstream-agent behavior diverged from the Claude baseline.
...
Most release mechanics are CI-enforced (`check_version_consistency.py` keeps `AGENTS.md` / SKILL.md / CHANGELOG / plugin manifests / README badge in lockstep; the release-cooldown workflow paces tags). One convention is editorial and lives here:
